...
pub async fn announce_to_tracker(
    ...
    let url = format!(
        "{}?info_hash={}&peer_id={}",
        torrent.metainfo.announce, torrent.encoded_info_hash, torrent.encoded_peer_id,
    );
        ...
        match value {
            BencodeValue::Dictionary(dict) => {
                ...
                let peers = match dict.get(b"peers".as_ref()) {
                    ...
                    Some(BencodeValue::ByteString(bytes)) => parse_compact_ipv4_peer_list(bytes)?,
...
impl BPeer {
    fn from_bencode_dict(dict: &BTreeMap<&[u8], BencodeValue>) -> Result<Self, String> {
        ...
        let ip: IpAddr = str::from_utf8(ip_string)
